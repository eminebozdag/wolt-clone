    ...
    <nav className="flex flex-col w-full">
      ...
      >
        <div className="flex flex-row justify-between items-center w-full h-[21px]">
          <span className="text-xxs text-ellipsis cursor-pointer overflow-hidden">
            {data.title}
          </span>

          {
            <DownArrow
              height={20}
              width={20}
              className={clsx("fill-c-white/50", isExpanded && "rotate-180")}
            />
          }
        </div>
      </Button>
      <div
        className={clsx(
          "text-xxs overflow-hidden",
          isExpanded ? "pl-[30px]" : "h-0"
        )}
      >
          ...
          {data.tags?.map((tag) => {
            return (
              <li>
                <a
                  href={tag.route}
                  className="text-c-white text-xxs cursor-pointer hover:underline"
                >
                  {tag.tag_title}
                </a>
              </li>
